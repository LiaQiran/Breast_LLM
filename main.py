#%%
import sys
sys.path.append(r'C:/Users/Qiran/Downloads/MedSAM-main/')
import numpy as np
...
npz_tr_path = 'data/BUS_dataset/train_vit_b'
...
for epoch in range(num_epochs):
    ...
    for step, (image_embedding, gt2D, boxes) in enumerate(tqdm(train_dataloader)):
        ...
        # Resize the predicted mask to match the ground truth size (512x512) using nearest neighbor interpolation
        #mask_predictions_resized = F.interpolate(mask_predictions, size=(512, 512), mode='nearest')
...
ts_img_path = 'data/BUS_dataset/test/images'
ts_gt_path = 'data/BUS_dataset/test/labels'
...
test_names1=test_names
# test_names1= [filename.replace('.png', '.bmp') for filename in test_names1]

gt_data = io.imread(join(ts_gt_path, test_names1[img_idx]))
...
ori_sam_predictor.set_image(image_data_pre)#processes the input image, resizing it internally to a fixed resolution for the model.
ori_sam_seg, _, _ = ori_sam_predictor.predict(point_coords=None, box=bbox_raw, multimask_output=False)#ori_sam_predictor.predict() takes a bounding box (bbox_raw) and internally produces a segmentation mask at 256x256.
...
with torch.no_grad():
    ...
    print(f"Original predicted mask shape: {medsam_seg.shape}")

    # Resize the predicted mask to the original image size (H, W)
    medsam_seg_resized = F.interpolate(
        torch.tensor(medsam_seg[None, None, :, :], dtype=torch.float32),  # Add batch and channel dimensions
        size=(H, W),  # Resize to original image size
        mode='nearest'  # Use nearest neighbor to preserve binary values (0, 1)
    ).squeeze().numpy().astype(np.uint8)  # Remove batch and channel dimensions and convert to NumPy array

    print(f"Resized mask shape: {medsam_seg_resized.shape}")
...
medsam_dsc = compute_dice_coefficient(gt_data > 0, medsam_seg_resized > 0)
...
show_mask(medsam_seg_resized, axs[2])
